...
#define DebugFOResolution [[[[NSProcessInfo processInfo] environment] objectForKey: @"DebugIonTheme"] boolValue]
#define DebugFOResolutionEmpty [[[[NSProcessInfo processInfo] environment] objectForKey: @"DebugIonThemeEmpty"] boolValue]

// Log Functions
#define FOReport(args...) if( DebugFOResolution ) NSLog(@"%s - %@",__PRETTY_FUNCTION__, [NSString stringWithFormat: args]);
#define FOReportEmpty(args...) if( DebugFOResolutionEmpty ) FOReport(args);
...
- (NSRegularExpression *)expressionForKey:(id) key {
    ...
    if ( [value isKindOfClass: [NSString class]] )
        ...
        if ( !type || ![type isKindOfClass: [NSString class]] ) {
            ...
            type = sFORegularExpression_type_Literal; // Default to the content being literal
        }
        ...
        // Create pattern
        if ( [type.uppercaseString isEqualToString: sFORegularExpression_type_Inclusive] )
            return [content toInclusiveExpression];
        else if ( [type.uppercaseString isEqualToString: sFORegularExpression_type_Exclusive] )
            return [content toExclusiveExpression];
        else // Literal
            return [content toExpresion];
    }
    else {
        FOReport( @"%@ is not a valid type for regular expression generation.", NSStringFromClass( [value class] ));
...
- (CGRect) toVec4UsingX1:(id) x1key y1:(id) y1Key x2:(id) x2Key andY2:(id) y2Key {
    ...
    return (CGRect){ (CGPoint){[x1 floatValue], [y1 floatValue]}, (CGSize){[x2 floatValue], [y2 floatValue]}};
}
