...
- (NSMutableDictionary*)instances {
  if (!_instances) _instances = [[NSMutableDictionary alloc] init];
  return _instances;
}

#pragma mark Target Time

+ (BOOL)automaticallyNotifiesObserversOfTargetTime {
  return FALSE;
}
...
- (void)stageNewInstanceWithCurrent {
  ...
  [self performBlock:^{
      ...
      if (![self.instances objectForKey:instanceID]) return;

      // Check if the target time is now
      if (self.targetTime < [NSDate date].timeIntervalSinceReferenceDate)
        [self invoke];  // invoke the action
      else
        [self stageNewInstanceWithCurrent];  // stage a new instance to meet our
                                             // goal.
