                    ...
                    andData:(NSDictionary *)data;
{
  ...
  if (!url || ![url isKindOfClass:[NSURL class]] || !reason ||
...
- (instancetype)initWithURL:(NSURL *)url
  ...
  return
      [self initWithURL:url reason:reason referralLink:referral andData:NULL];
}
...
+ (instancetype)referralLinkWithURL:(NSURL *)url {
  return
      [[[self class] alloc] initWithURL:url andReason:sIACLinkReason_Referral];
}
...
+ (instancetype)referralLinkWithURLString:(NSString *)urlString {
  return [[[self class] alloc] initWithURLString:urlString
                                       andReason:sIACLinkReason_Referral];
}
...
- (void)getPathComponentsFromURL {
  ...
  components =
      [[NSMutableArray alloc] initWithArray:_url.pathComponents copyItems:TRUE];
...
- (NSString *)toString {
  return [NSString
      stringWithFormat:
          @"URL: \"%@\", Reason: \"%@\", Referral: \"%@\", Has Data: %@",
          [self urlString], self.reason ? self.reason : @"",
...
+ (NSString *)pathStringFromComponents:(NSArray *)components {
  ...
  for (NSString *component in components)
    resultString =
        [NSString stringWithFormat:@"%@%@/", resultString, component];
...
+ (NSString *)urlStringFromComponents:(NSArray *)components {
  return [[self class]
      urlStringFromPath:[[self class] pathStringFromComponents:components]];
}

+ (NSString *)urlStringFromPath:(NSString *)path {
  return [NSString
      stringWithFormat:@"%@://%@", [[self class] standardSchema], path];
}
...
+ (NSDictionary *)queryParametersFromString:(NSString *)string {
  ...
  for (NSString *component in queryComponents) {
    ...
    if (equalsLocation.location == NSNotFound)
      ...
      // Get the key.
      key = [[self class]
          decodeURLString:[component substringToIndex:equalsLocation.location]];
      ...
      if (value) {
        ...
        if ([(NSString *)value conformsToExpression:[@"1234567890.-" toInclusiveExpression]]) {
          result = [numberFormatter numberFromString:value];
