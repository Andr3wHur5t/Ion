      ...
      initWithPath:
          [[IonPath bundleDirectory]
              pathAppendedByElement:
                  [name stringByAppendingString:sIonAnimationFileExtension]]];
}
...
- (instancetype)initWithPath:(IonPath *)path {
  ...
  if (!file || ![file isKindOfClass:[NSData class]]) {
    NSLog(@"%s - File does not exist! %@", __PRETTY_FUNCTION__, path);
    return NULL;
  }

  animationConfig = [file toJsonDictionary];
  if (!animationConfig ||
      ![animationConfig isKindOfClass:[NSDictionary class]]) {
...
- (void)setRawConfig:(NSDictionary *)rawConfig {
  NSParameterAssert(rawConfig &&
                    [rawConfig isKindOfClass:[NSDictionary class]]);
  if (!rawConfig || ![rawConfig isKindOfClass:[NSDictionary class]]) return;

  _rawConfig = rawConfig;

  // Update our internal states
  [self.frames
      setRawData:[_rawConfig dictionaryForKey:sIonAnimationMap_FramesKey]];
}
...
- (IonAccessBasedGenerationMap *)frames {
  if (!_frames) _frames = [[IonAccessBasedGenerationMap alloc] init];
  return _frames;
}
...
- (id (^)(id data))frameGenerationBlock {
  return ^id(id data) {
    if (!data || ![data isKindOfClass:[NSDictionary class]]) return NULL;
...
+ (void)addMapToCache:(IonAnimationMap *)map withName:(NSString *)name {
  ...
  NSParameterAssert(name && [name isKindOfClass:[NSString class]]);
  if (!map || ![map isKindOfClass:[IonAnimationMap class]] || !name ||
