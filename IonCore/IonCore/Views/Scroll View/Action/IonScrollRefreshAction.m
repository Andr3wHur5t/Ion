...
- (instancetype)init {
  ...
  if (self) {
    ...
    // Configurable
    self.canInvokeActionSets =
        sIonScrollRefreshActionDefault_CanInvokeActionSets;
    self.distance = sIonScrollRefreshActionDefault_Distance;
    self.returnBuffer = 0.0f;
  }
  return self;
}
...
- (instancetype)initWithRequiredDistance:(CGFloat)distance {
  self = [super init];
  if (self) self.distance = distance;
  return self;
}
...
- (void)setStatusDelegate:
        (id<IonScrollRefreshActionStatusDelegate>)statusDelegate {
  if (!statusDelegate)
    _statusDelegate = statusDelegate;
  else if ([(NSObject *)statusDelegate
               conformsToProtocol:@protocol(
                                      IonScrollRefreshActionStatusDelegate)]) {
    _statusDelegate = statusDelegate;
...
- (void)updateDelegateWithPercentage:(CGFloat)percentage {
  self.statusDelegate.gestureCompletionPercentage = lastDelegateUpdateValue =
      percentage;
}
...
- (void)scrollViewDidUpdateContext:(IonScrollView *)scrollView {
  NSParameterAssert(scrollView &&
                    [scrollView isKindOfClass:[IonScrollView class]]);
  if (!scrollView || ![scrollView isKindOfClass:[IonScrollView class]]) return;

  /** Update Checks */
  // A refresh can be initialize by pulling the scroll view down past the
  // threshold, when a
  ...
  } else if (ceil(scrollView.contentOffset.y) >=
             floor(self.returnBuffer * -1))  // Did we reset?
    _canInternalyRefresh = true;  // We have reset enough, now we are no longer
                                  // in a internal refresh.

  _wasTracking =
      scrollView.tracking;  // Update previous state with current one.

  /** Delegate Updates */
  // Update our status delegate with our current percentage to completion
  if (self.statusDelegate) {
    if (scrollView.contentOffset.y <= 0)
      [self updateDelegateWithPercentage:(CGFloat)MIN(
                                             ABS(scrollView.contentOffset.y) /
                                                 ABS(self.distance),
                                             1.0)];
