...
/**
 * Gets invoked by the guide set when one of the guides describing the content size changes.
 */
...
- (IonGuideLine *)contentOffsetHoriz {
    if ( !_contentOffsetHoriz )
        _contentOffsetHoriz = [IonGuideLine guideFromPointOnTarget: self
...
- (IonGuideLine *)contentOffsetVert {
    if ( !_contentOffsetVert )
        _contentOffsetVert = [IonGuideLine guideFromPointOnTarget: self
                                                     usingKeyPath: @"contentOffset"
...
- (IonGuideSet *)contentSizeGuideSet {
    if ( !_contentSizeGuideSet ) {
        ...
        [_contentSizeGuideSet addTarget: self andAction: @selector(contentSizeGuidesDidChange)];
    }
...
- (void) setContentSize:(CGSize) contentSize {
    ...
    // Intercept the last captured manual content size so we can revert to it if necessary
...
+ (NSSet *)keyPathsForValuesAffectingContentSizeHoriz {
    return [[NSSet alloc] initWithArray: @[ @"contentSizeGuideSet.contentSizeHoriz" ]];
}

- (void) setContentSizeHoriz:(IonGuideLine *)contentSizeHoriz {
    [self.contentSizeGuideSet setGuide: contentSizeHoriz forKey: @"contentSizeHoriz"];
}
...
+ (NSSet *)keyPathsForValuesAffectingContentSizeVert {
    return [[NSSet alloc] initWithArray: @[ @"contentSizeGuideSet.contentSizeVert" ]];
}
...
- (void) contentSizeGuidesDidChange {
    ...
    // We need to set it to the super so we can intercept when the user sets the content size manually.
...
- (void) applyScrollConfiguration:(NSDictionary *)config {
    ...
    // Bounce at edge
    self.bounces = [config boolForKey: sIonScrollStyle_BouncesAtEdge defaultValue: TRUE];
    
    // always bounce vertical
    self.alwaysBounceVertical = [config boolForKey: sIonScrollStyle_AlwaysBouncesVertical defaultValue: FALSE];
    
    // always bounce horizontal
    self.alwaysBounceHorizontal = [config boolForKey: sIonScrollStyle_AlwaysBouncesHorizontal defaultValue: FALSE];
    
    // scroll to top gesture enabled
    self.scrollsToTop = [config boolForKey: sIonScrollStyle_UsesScrollToTopGesture defaultValue: TRUE];
    
    // Deceleration rate
    self.decelerationRate = [config scrollViewDecelerationRateForKey: sIonScrollStyle_DecelerationRate];
    
    // Keyboard dismiss mode
    self.keyboardDismissMode = [config scrollViewKeyboardDismissModeForKey: sIonScrollStyle_KeyboardDismissMode];
}
...
- (void) applyScrollIndicatorsConfiguration:(NSDictionary *)config {
    if ( !config || ![config isKindOfClass: [NSDictionary class]] )
        return;
    // Indicator style
    self.indicatorStyle = [config scrollViewIndicatorStyleForKey: sIonScrollStyle_IndicatorStyle];
    
    // Shows horizontal scroll indicator
    self.showsHorizontalScrollIndicator = [config boolForKey: sIonScrollStyle_ShowsHorizontalIndicator
                                                defaultValue: TRUE];
    
    // Shows vertical scroll indicator
    self.showsVerticalScrollIndicator = [config boolForKey: sIonScrollStyle_ShowsHorizontalIndicator
                                              defaultValue: TRUE];
    
    // Scroll Indicator insets
    self.scrollIndicatorInsets = [config edgeInsetsForKey: sIonScrollStyle_IndicatorInsets];
}
...
- (void) applyZoomConfiguration:(NSDictionary *)config {
    ...
    // Zoom bounces
    self.bouncesZoom = [config boolForKey: sIonScrollStyle_ZoomBounces defaultValue: TRUE];
}
