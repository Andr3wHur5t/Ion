...
@interface IonTextField () {
  CGFloat _iAlpha;  // ivar to hold our user set alpha so we can animate alpha
                    // while preserving user set states.
...
@property(strong, nonatomic, readonly) id<UITextFieldDelegate> textDelegate;
...
- (void)setTextDelegate:(id<UITextFieldDelegate>)textDelegate {
  [self willChangeValueForKey:@"textDelegate"];
  _textDelegate = textDelegate;
  self.delegate = _textDelegate;
  [self didChangeValueForKey:@"textDelegate"];
}

#pragma mark Input Filter

- (IonInputFilter *)inputFilter {
  if (!_inputFilter) _inputFilter = [[IonInputFilter alloc] init];
  return _inputFilter;
}
...
- (void)applyStyle:(IonStyle *)style {
  ...
  self.placeholderFont =
      [style.configuration fontForKey:sIonThemeText_PlaceholderFont];

  // Placeholder Color
  self.placeholderTextColor =
      [style.configuration colorForKey:sIonThemeText_PlaceholderColor
                            usingTheme:style.theme];

  /** Input */
  // Input Filter
  self.inputFilter =
      [style.configuration inputFilterForKey:sIonTextField_InputFilterKey];

  // Set bahaviors
  self.behaviorDictionary =
      [style.configuration dictionaryForKey:sIonTextField_ResignBehaviorKey];

  // Configure keyboard
  [self processKeybordConfiguration:
            [style.configuration dictionaryForKey:sIonTextField_KeyboardKey]];
}
...
- (void)processKeybordConfiguration:(NSDictionary *)config {
  ...
  self.keyboardAppearance =
      [config keyboardAppearanceForKey:sIonTextField_KeyboardAppearanceKey];

  // Auto Correct type
  self.autocorrectionType =
      [config autocorrectionTypeForKey:sIonTextField_KeyboardAppearanceKey];

  // Spell Check type
  self.spellCheckingType =
      [config spellcheckTypeForKey:sIonTextField_SpellCheckTypeKey];

  // Auto Capitalization Type
  self.autocapitalizationType = [config
      autocapitalizationTypeForKey:sIonTextField_AutoCapitalizationTypeKey];

  // Return Key
  self.returnKeyType =
      [config returnKeyTypeForKey:sIonTextField_ReturnKeyTypeKey];
...
- (void)inputDidFailFilter {
  [UIView animateWithDuration:self.animationDuration / 2
      animations:^{
          if (super.alpha != _iAlpha) return;
          super.alpha = _iAlpha - 0.4f;
      }
      completion:^(BOOL finished) {
          if (!finished) return;
          [UIView animateWithDuration:self.animationDuration / 2
                           animations:^{ super.alpha = _iAlpha; }];
      }];
}
...
- (void)addTarget:(id)target action:(SEL)action {
  [self.enterKeyTargetActionList addTarget:target
                                 andAction:action
                                   toGroup:@"all"];
}

- (void)removeTarget:(id)target action:(SEL)action {
  [self.enterKeyTargetActionList removeTarget:target
                                    andAction:action
                                    fromGroup:@"all"];
}
...
- (void)inputReturnKeyDidGetPressed {
  [self resignFirstResponderWithReason:
            sIonTextFieldBehavior_Reason_ReturnHit];  // Attempt to resign using
                                                      // the return key action.
...
- (BOOL)resignFirstResponder {
  ...
  if (self.canResignExternally)
    return [self resignFirstResponderWithReason:
...
- (BOOL)resignFirstResponderWithReason:(NSString *)action {
  if ([self canResignFirstResponderWithoutInvalidCheckForReason:action])
    return [self forcivlyResignFirstResponder];  // We don't need to run
                                                 // validation to resign.
  else if ([self textIsValid])
    return
        [self forcivlyResignFirstResponder];  // Text is valid, we can resign.
  else {  // Text isn't valid, and we are required to have valid text to return.
    [self invokeErrorAnimationForAction:action];  // Present a validation error
                                                  // animation in the UI.
...
+ (BOOL)automaticallyNotifiesObserversOfBehaviorDictionary {
  return FALSE;
}

- (void)setBehaviorDictionary:(NSDictionary *)behaviorDictionary {
  if (!behaviorDictionary ||
      ![behaviorDictionary isKindOfClass:[NSDictionary class]])
    return;
...
- (NSDictionary *)behaviorDictionary {
  if (!_behaviorDictionary)  // if the dictionary is empty, construct with the
                             // default one.
    _behaviorDictionary = @{
...
- (BOOL)canResignFirstResponderWithoutInvalidCheckForReason:(NSString *)action {
  ...
  if (!action || ![action isKindOfClass:[NSString class]]) return FALSE;

  return [[self.behaviorDictionary dictionaryForKey:action]
      boolForKey:sIonTextField_ResignBehavior_NoValidation];
}
...
- (void)invokeErrorAnimationForAction:(NSString *)action {
  ...
  if (animationPointer) [self startAnimationWithPointerMap:animationPointer];
}
...
- (void)setPlaceholder:(NSString *)placeholder {
  if ((!_placeholderTextColor && !_placeholderFont) || !placeholder)
    ...
    super.attributedPlaceholder = [[NSAttributedString alloc]
...
- (NSDictionary *)placeholderAttributes {
  return @{
    NSForegroundColorAttributeName : self.placeholderTextColor
                                         ? self.placeholderTextColor
                                         : [UIColor grayColor],
    NSFontAttributeName : self.placeholderFont ? self.placeholderFont
                                               : self.font
  };
}
...
+ (BOOL)automaticallyNotifiesObserversOfPlaceholderTextColor {
  return FALSE;
}
...
+ (BOOL)automaticallyNotifiesObserversOfPlaceholderFont {
  return FALSE;
}
