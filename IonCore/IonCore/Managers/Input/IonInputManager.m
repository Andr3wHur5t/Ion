...
/**
 * Gets called when a textField was provided input, where the replacement string
 * is the string to set, or append.
 * @return {BOOL} stating if the text field should allow the change to occur.
 */
- (BOOL)textField:(UITextField *)textField
  ...
  if ([textField conformsToProtocol:@protocol(IonInputManagerFilterSpec)]) {
    ...
    // If it doesn't report an error to the view so it can respond
    // appropriately.
    if (!canUseReplacement)
      if ([textField respondsToSelector:@selector(inputDidFailFilter)])
        [textField performSelector:@selector(inputDidFailFilter)
                        withObject:NULL];  // Report that we did fail filter
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
  if (!textField || ![textField isKindOfClass:[UITextField class]]) return TRUE;
  ...
  if ([textField conformsToProtocol:@protocol(IonInputManagerFilterSpec)] &&
    ...
    [textField performSelector:@selector(inputReturnKeyDidGetPressed)
                    withObject:NULL];
  return TRUE;
}
