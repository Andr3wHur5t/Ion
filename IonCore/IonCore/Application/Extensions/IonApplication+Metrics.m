 ...
 */
static NSString* sIonApplication_Metrics_StartupInitTime =
    @"Metrics_StartupInitTime";
static NSString* sIonApplication_Metrics_ApplicationLaunchBeginTime =
    @"Metrics_ApplicationLaunchBeginTime";
static NSString* sIonApplication_Metrics_ApplicationLaunchEndTime =
    @"Metrics_ApplicationLaunchEndTime";
static NSString* sIonApplication_Metrics_SplashDisplayCallbackTime =
    @"Metrics_SplashDisplayCallbackTime";
...
/**
 * Gets a startup metrics pretty report screen.
 * @return {NSString*}
 */
- (NSString*)startupMetricsReportString {
  ...
  report = [@"\n" stringByAppendingString:
                      @"Time from init to launch: %.2f ms (Shared with OS)\n"];
  report = [report stringByAppendingString:@"Time spent launching: %.2f ms\n"];
  report = [report
      stringByAppendingString:@"Time generating rapid splash: %.2f ms\n"];
  report = [report
      stringByAppendingString:@"Time To first rapid splash render: %.2f ms\n"];

  // Fill out the report
  report = [NSString
      stringWithFormat:report, SecondsToMilliseconds(self.ionInitToLaunchTime),
                       SecondsToMilliseconds(self.ionLaunchFunctTime),
                       SecondsToMilliseconds(self.ionInitToSplashGenerationEnd),
                       SecondsToMilliseconds(
                           self.ionLaunchToSplashDisplayTime)];
...
+ (BOOL)automaticallyNotifiesObserversOfStartupInitTime {
  return FALSE;
}
...
- (NSTimeInterval)startupInitTime {
  return [[self.categoryVariables
      numberForKey:sIonApplication_Metrics_StartupInitTime] doubleValue];
}

#pragma mark ApplicationLaunchBeginTime

+ (BOOL)automaticallyNotifiesObserversOfApplicationLaunchBeginTime {
  return FALSE;
}

- (void)setApplicationLaunchBeginTime:
        (NSTimeInterval)applicationLaunchBeginTime {
  [self willChangeValueForKey:@"applicationLaunchBeginTime"];
  [self.categoryVariables
      setObject:[NSNumber numberWithDouble:applicationLaunchBeginTime]
...
- (NSTimeInterval)applicationLaunchBeginTime {
  return [[self.categoryVariables
      numberForKey:
          sIonApplication_Metrics_ApplicationLaunchBeginTime] doubleValue];
}

#pragma mark ApplicationLaunchEndTime

+ (BOOL)automaticallyNotifiesObserversOfApplicationLaunchEndTime {
  return FALSE;
}
...
- (void)setApplicationLaunchEndTime:(NSTimeInterval)applicationLaunchEndTime {
  ...
  [self.categoryVariables
      setObject:[NSNumber numberWithDouble:applicationLaunchEndTime]
...
- (NSTimeInterval)applicationLaunchEndTime {
  return [[self.categoryVariables
      numberForKey:
          sIonApplication_Metrics_ApplicationLaunchEndTime] doubleValue];
}

#pragma mark ApplicationLaunchEndTime
+ (BOOL)automaticallyNotifiesObserversOfSplashDisplayCallbackTime {
  return FALSE;
}
...
- (void)setSplashDisplayCallbackTime:(NSTimeInterval)splashDisplayCallbackTime {
  ...
  [self.categoryVariables
      setObject:[NSNumber numberWithDouble:splashDisplayCallbackTime]
...
- (NSTimeInterval)splashDisplayCallbackTime {
  return [[self.categoryVariables
      numberForKey:
          sIonApplication_Metrics_SplashDisplayCallbackTime] doubleValue];
}
