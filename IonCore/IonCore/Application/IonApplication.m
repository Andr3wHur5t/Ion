...
- (UIWindow *)constructWindow {
  ...
  if (![[self class] isInDemoMode])
    ...
    window =
        [[IonDemoUIWindow alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
  return window;
}
...
- (void)setupApplication {
  NSAssert(TRUE,
           @"IonApplication - (void)setupApplication; was not overloaded!");
}
...
- (void)configureStatusBar {
  NSAssert(TRUE,
           @"IonApplication - (void)configureStatusBar; was not overloaded!");
}
...
- (CGFloat)sizeModifier {
  return [UIScreen mainScreen].scale >= 3.0f ? 1.333 : 1;
}
...
+ (instancetype)sharedApplication {
  ...
  if (![currentDelegate isKindOfClass:[self class]]) return NULL;
