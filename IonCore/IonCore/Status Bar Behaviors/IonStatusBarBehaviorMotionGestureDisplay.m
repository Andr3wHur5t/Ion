...
/**
 * The timing mechanism for display
 */
...
/**
 * Animates the status bar to the correct state using the set animation. Same As
 * Toggle mode.
 */
- (void)updateToggleMode {
  [[IonApplication sharedApplication]
      setStatusBarHidden:![IonApplication sharedApplication].statusBarHidden
...
- (void)motionEnded:(UIEventSubtype)motion withEvent:(UIEvent*)event {
  if (motion == self.targetMotionType) [self updateState];
}
@end
