...
@interface IonGuideTargetToken () {
  // There is an issue with auto synth on blocks, we neet to keep this as an
  // ivar.
...
+ (instancetype)tokenWithObservedObject:(id)object
                             andKeyPath:(NSString *)keyPath {
  return
      [[[self class] alloc] initWithObservedObject:object andKeyPath:keyPath];
}
...
- (id)rawValue {
  if (!self.observedObject || !self.keyPath ||
      ![self.keyPath isKindOfClass:[NSString class]]) {
    NSAssert(true, @"Can't get value, did not init with valid values. key "
                   @"path: %@, observed object: %@",
...
- (void)configureObserverWithTarget:(id)target andSelector:(SEL)selector {
  ...
  if (!self.keyPath || !self.observedObject) {
    NSAssert(true, @"Can't configure, did not init with valid values. key "
                   @"path: %@, observed object: %@",
             self.keyPath, self.observedObject);
    return;
  }
  _observer = [FOKeyValueObserver
      observeObject:self.observedObject
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"observing %@ on %@", self.keyPath,
                       NSStringFromClass([self.observedObject class])];
}
...
+ (IonGuideLineProcessingBlock)defaultProcessingBlock {
  return ^CGFloat(id data) {
    if (![data respondsToSelector:@selector(toFloat)]) return 0.0f;
...
+ (IonGuideLineProcessingBlock)rectSizeProcessingBlockUsingMode:
        (IonGuideLineFrameMode)mode {
  return ^CGFloat(id data) {
    CGSize size;
    if (!data || ![data isKindOfClass:[NSValue class]]) return 0.0f;
...
+ (IonGuideLineProcessingBlock)sizeProcessingBlockUsingMode:
                                   (IonGuideLineFrameMode)mode
...
+ (IonGuideLineProcessingBlock)pointProcessingBlockUsingMode:
                                   (IonGuideLineFrameMode)mode
                                                   andAmount:(CGFloat)amount {
  return ^CGFloat(id data) {
    CGPoint point;
...
+ (IonGuideLineProcessingBlock)externalPositioningProcessingBlockUsingMode:
        (IonGuideLineFrameMode)mode {
  return [[self class] externalPositioningProcessingBlockUsingMode:mode
                                                         andAmount:1.0f];
}

+ (IonGuideLineProcessingBlock)
    externalPositioningProcessingBlockUsingMode:(IonGuideLineFrameMode)mode
                                      andAmount:(CGFloat)amount {
  return ^CGFloat(id data) {
    CGRect frame;
    if (!data || ![data isKindOfClass:[NSValue class]]) return 0.0f;
    frame = [(NSValue *)data CGRectValue];
    return ((mode == IonGuideLineFrameMode_Vertical ? frame.size.height
                                                    : frame.size.width) *
            amount) +
           (mode == IonGuideLineFrameMode_Vertical ? frame.origin.y
                                                   : frame.origin.x);
  };
}
