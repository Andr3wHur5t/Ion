...
- (instancetype)initWithTargetToken:(IonGuideTargetToken *)targetToken
  ...
  NSParameterAssert(targetToken &&
                    [targetToken isKindOfClass:[IonGuideTargetToken class]]);
...
- (instancetype)initWithTarget:(id)target
  ...
  if (!target || !keyPath || ![keyPath isKindOfClass:[NSString class]])
    return NULL;
  return [self initWithTargetToken:[IonGuideTargetToken
                                       tokenWithObservedObject:target
                                                       keyPath:keyPath
...
- (instancetype)initWithTarget:(id)target andKeyPath:(NSString *)keyPath {
  return [self initWithTarget:target
                      keyPath:keyPath
              processingBlock:NULL
                 andCalcBlock:NULL];
...
- (void)configureGuideLineAsChild:(IonGuideLine *)line {
  ...
  if (!line || ![line isKindOfClass:[IonGuideLine class]]) return;
 ...
 * @param {id} the target object.
 * @param {NSString*} the key to observe.
 * @param {IonGuideLineProcessingBlock} the processing block used to extract the
 * variable.
 */
- (void)setTarget:(id)target
  ...
  NSParameterAssert(target);
  NSParameterAssert(keyPath && [keyPath isKindOfClass:[NSString class]]);
  if (!target || !keyPath || ![keyPath isKindOfClass:[NSString class]]) return;
  [self setPrimaryTargetToken:[IonGuideTargetToken
                                  tokenWithObservedObject:target
                                                  keyPath:keyPath
                                       andProcessingBlock:processingBlock]];
}
...
/**
 * Gets the calculation block, if it doesn't exist replace it with the default
 * one.
 * @return {IonGuildLineCalcBlock} the calculation block, or default calculation
 * block if it has not been set
 */
- (IonGuildLineCalcBlock)calcBlock {
  if (!_calcBlock) _calcBlock = [IonGuideLine defaultCalculationBlock];
  return _calcBlock;
}
...
- (void)setCalcBlock:(IonGuildLineCalcBlock)calcBlock {
  // Note: we can't use an auto synthesized value... look at the notes in the
  // ivar definition.
...
- (NSMutableDictionary *)targets {
  if (!_targets) _targets = [[NSMutableDictionary alloc] init];
  return _targets;
}
 ...
 */
- (void)addTargetToken:(IonGuideTargetToken *)targetToken
              withName:(NSString *)name {
  NSParameterAssert(name && [name isKindOfClass:[NSString class]]);
  NSParameterAssert(targetToken &&
                    [targetToken isKindOfClass:[IonGuideTargetToken class]]);
  if (!name || ![name isKindOfClass:[NSString class]] || !targetToken ||
      ![targetToken isKindOfClass:[IonGuideTargetToken class]])
    return;

  // Configure it to call the guide when it observes a change
  [targetToken configureObserverWithTarget:self
                               andSelector:@selector(targetDidChange:)];
...
- (void)addTarget:(id)target
  ...
  [self addTargetToken:[IonGuideTargetToken
                           tokenWithObservedObject:target
                                           keyPath:keyPath
                                andProcessingBlock:processingBlock]
              withName:name];
...
- (void)targetDidChange:(NSDictionary *)change {
  ...
  if (!change || ![change isKindOfClass:[NSDictionary class]]) return;
...
/**
 * Constructs a dictionary representing all current targets values with their
 * associated name.
 * @return {NSDictionary*}
 */
- (NSDictionary *)currentTargetValues {
  ...
  [self.targets enumerateKeysAndObjectsUsingBlock:^(id key, id obj,
                                                    BOOL *stop) {
      [currentValues
          setObject:[NSNumber numberWithFloat:(float)((IonGuideTargetToken *)
                                                          obj).processedValue]
             forKey:key];
...
- (void)recalculatePosition {
  // Gets a dictionary representing all target values, then passes that into our
  // calculation block which
  // returns a float. That value is then passed to update position with performs
...
- (void)updatePosition:(CGFloat)newPosition {
  ...
  if (self.position == newPosition) return;
  ...
  if (_debugBlock) _debugBlock();
}

#pragma mark Local Observers
/**
 * Gets, or constructs the local targets dictionary.
 * @return {FOTargetActionList*}
 */
- (FOTargetActionList *)localObservers {
  if (!_localObservers) _localObservers = [[FOTargetActionList alloc] init];
...
- (void)removeLocalObserverTarget:(id)target andAction:(SEL)action {
  [self.localObservers removeTarget:target
                          andAction:action
                          fromGroup:@"observers"];
}
...
/**
 * Constructs a guide line as a child of the current guide line with the
 * specified calculation block.
 * @param {IonGuildLineCalcBlock} the calculation block used to calculate the
 * resulting position.
 */
- (instancetype)guideAsChildUsingCalcBlock:(IonGuildLineCalcBlock)calcBlock {
  return [[self class] guideWithTarget:self
                               keyPath:@"position"
                          andCalcBlock:calcBlock];
}
 ...
 * @param {IonGuideLineProcessingBlock} the block to use in processing the
 * watched value on updates.
 * @param {IonGuildLineCalcBlock} the calculation block used to calculate the
 * resulting position.
 */
 ...
 * @param {NSString*} the key path of the value to watch on the target object.
 */
+ (instancetype)guideWithTarget:(id)target andKeyPath:(NSString *)keyPath {
  return [[self alloc] initWithTarget:target
                              keyPath:keyPath
                      processingBlock:NULL
                         andCalcBlock:NULL];
...
+ (IonValueModBlock)modBlockForType:(IonValueModType)type {
  switch (type) {
    ...
    case IonValueModType_Divide:
      return ^CGFloat(CGFloat a, CGFloat b) { return a / b; };
      break;
    case IonValueModType_Max:
      return ^CGFloat(CGFloat a, CGFloat b) { return MAX(a, b); };
      break;
    case IonValueModType_Min:
      return ^CGFloat(CGFloat a, CGFloat b) { return MIN(a, b); };
      break;
    default:
      return ^CGFloat(CGFloat a, CGFloat b) { return a + b; };
      break;
 ...
 * @param {CGFloat(^)( CGFloat a, CGFloat b)} the block which gets called for
 * each set of values.
 * @return {IonGuildLineCalcBlock}
 */
+ (IonGuildLineCalcBlock)compositeCalcBlockUsingValueBlock:
                             (IonValueModBlock)valueBlock {
...
+ (IonGuildLineCalcBlock)calcBlockForModType:(IonValueModType)modType {
  return [[self class]
      compositeCalcBlockUsingValueBlock:[[self class] modBlockForType:modType]];
}
 ...
 * @return {IonGuildLineCalcBlock}
 */
+ (IonGuildLineCalcBlock)calcBlockWithSubtracted:(NSString *)value1
                                              by:(NSString *)value2 {
  ...
  if (!value1 || ![value1 isKindOfClass:[NSString class]] || !value2 ||
