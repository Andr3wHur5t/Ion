...
- (void)setGuideSet:(IonViewGuideSet *)guideSet {
  objc_setAssociatedObject(self, sIonGuideSetKey, guideSet,
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

/**
 * The guide set getter, creates it if it doesn't exist
 ...
 */
- (void)setLogAutoFrameUpdatesWithString:
        (NSString *)logAutoFrameUpdatesWithString {
  if (logAutoFrameUpdatesWithString)
    [self.categoryVariables setObject:logAutoFrameUpdatesWithString
                               forKey:sIonAutoUpdateLogKey];
  else
...
/**
 * Logs the guide set if applicable
 */
- (void)logAutoFrameChange {
  NSString *logKey;

  logKey = self.logAutoFrameUpdatesWithString;
  if (logKey) NSLog(@"%@: %@", logKey, self.guideSet);
}
 ...
 */
- (void)setLocalGuidesWithHorz:(IonGuideLine *)horiz
                       andVert:(IonGuideLine *)vert {
 ...
 * @param vert - the vertical guide line.
 */
- (void)setSuperGuidesWithHorz:(IonGuideLine *)horiz
                       andVert:(IonGuideLine *)vert {
 ...
 */
- (void)setSizeHorizontalWithLeft:(IonGuideLine *)left
                         andRight:(IonGuideLine *)right {
 ...
 */
- (void)setSizeVerticalyWithTop:(IonGuideLine *)top
...
- (void)updateFrameToMatchGuideSet {
  ...
  // Check sizing rules
  if (CGSizeEqualToSize(frame.size, CGSizeZero)) frame.size = self.frame.size;
