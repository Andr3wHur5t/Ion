...
- (instancetype)init {
  self = [super init];
  if (self) _bundleManifest = [@{} mutableCopy];
  return self;
}
 ...
 * @param {IonCompletionBlock} the completion thats called once the manifest is
 * loaded.
 * @returns {instancetype}
 */
...
/**
 * Constructs an image manager using the inputed name as the bundle manifest,
 * and cache directory.
 * @param {NSString*} name
 ...
 */
+ (IonImageManager*)imageManagerWithName:(NSString*)name
                           andCompletion:(IonCompletionBlock)completion {
  return
      [[IonImageManager alloc] initWithName:name withLoadCompletion:completion];
}
...
- (IonPath*)bundleManifestPath {
  return [[IonPath bundleDirectory]
      pathAppendedByElement:
          [_groupName
              stringByAppendingString:sIonImageManagerBundleManifestExtension]];
}
  ...
  [NSDictionary dictionaryAtPath:[self bundleManifestPath]
                usingResultBlock:^(id returnedObject) {
                    NSMutableDictionary* dict =
                        [(NSDictionary*)returnedObject mutableCopy];

                    // Add the placeholder
                    [dict setObject:[IonImageManager placeholderItem]
                             forKey:sIonImageMaagerPlaceholderImageKey];
...
- (void)addImage:(UIImage*)image
                  forKey:(NSString*)key
    usingCompletionBlock:(IonCompletionBlock)completion {
  NSString* cleanKey = [NSDictionary sanitizeKey:key];
  [self setObject:[NSData dataFromImage:image]
              forKey:[cleanKey stringByAppendingString:
                                   sIonImageManagerStandardFileExtension]
...
- (void)imageForKey:(NSString*)key
    withReturnCallback:(IonImageReturn)returnCallback {
  ...
  // Validate
  if (!key || ![key isKindOfClass:[NSString class]] || !returnCallback) return;
  ...
  // Check Cache
  [self objectForKey:[key stringByAppendingString:
                      sIonImageManagerStandardFileExtension]
      usingGenerationBlock:[self imageGenBlock]
           withResultBlock:^(id object) {
               ...
               [self imageFromBundleUsingName:fileName
                               andResultBlock:^(UIImage* image) {
                                   resultingImage = image;
                                   //            [resultingImage setInfo:
                                   //            imageInfo];
                                   resultBlock(resultingImage);
                               }];
           }];
 ...
 */
- (void)imageFromBundleUsingName:(NSString*)name
...
/**
 * Gets a resized image using the key to retrieve it from the file system, or
 the cache.
...
- (void)imageForKey:(NSString*)key
  ...
  [self imageForKey:key
               withSize:size
               contined:TRUE
      andReturnCallback:returnCallback];
}
...
- (void)imageForKey:(NSString*)key
  ...
  // Validate
  if (CGSizeEqualToSize(size, CGSizeUndefined) ||
      CGSizeEqualToSize(size, CGSizeZero) || !returnCallback || !key ||
      ![key isKindOfClass:[NSString class]])
    return;

  // Get the type
  typeString = @"C";
  if (!isContained) typeString = @"F";

  // Get the string
  cleanKey = [NSDictionary sanitizeKey:key];
  normilizedSize = [SMUtilities normilizeSizeToScreen:size];
  sizedKeyString = [cleanKey
      stringByAppendingFormat:@"[%ix%i]-%@", (int)normilizedSize.width,
                              (int)normilizedSize.height, typeString];

  // Get the Image
  [self imageForKey:[sizedKeyString stringByAppendingString:
                                        sIonImageManagerStandardFileExtension]
...
- (void)resizeImage:(UIImage*)image
  ...
  if (!image || ![image isKindOfClass:[UIImage class]] || !key ||
      ![key isKindOfClass:[NSString class]] ||
      CGSizeEqualToSize(size, CGSizeUndefined) ||
      CGSizeEqualToSize(size, CGSizeZero)) {
    if (resultBlock) resultBlock(NULL);
    return;
  }
  ...
  cacheBlock = ^(UIImage* image) {
      ...
      [self addImage:image forKey:blockKey usingCompletionBlock:NULL];
  };
...
+ (IonImageManager*)interfaceManager {
  ...
  dispatch_once(&interfaceManager_OnceToken, ^{
      interfaceImageManeger = [IonImageManager
          imageManagerWithName:sIonInterfaceImageManagerCacheGroupName];
  });
