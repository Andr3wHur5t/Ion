 ...
 */
+ (IonSimpleCache*)cacheAtPath:(IonPath*)path
            withLoadCompletion:(IonCompletionBlock)manifestLoadCompletion {
  return [[IonSimpleCache alloc] initAtPath:path
                         withLoadCompletion:manifestLoadCompletion];
}
...
+ (IonSimpleCache*)cacheWithName:(NSString*)cacheName
              withLoadCompletion:(IonCompletionBlock)manifestLoadCompletion {
  return [[IonSimpleCache alloc] initWithName:cacheName
                           withLoadCompletion:manifestLoadCompletion];
}
...
- (void)dealloc {
  ...
  if (_manifestWillPersist)
    [self saveManifest:^(NSError* error) {
        if (!error) NSLog(@"Saved Cache Manifest At '%@' ", manifestPath);
    }];
  else
    [self deleteManifest:^(NSError* error) {
        if (!error) NSLog(@"Deleted Cache Manifest At '%@' ", manifestPath);
    }];
}
...
- (void)setObject:(id)object
    forKeyInMemory:(NSString*)key
    withCompletion:(IonRawDataSourceCompletion)completion {
...
- (void)loadManifest:(IonCompletionBlock)completion {
  ...
  [NSDictionary
      ...
      usingResultBlock:^(id returnedObject) {
          ...
          // Set the data
          [_manifest
              setDictionary:[returnedObject
                                overriddenByDictionaryRecursively:_manifest]];
...
- (void)deleteManifest:(IonCompletionBlock)completion {
  [IonFileIOmanager deleteItem:[self manifestPath]
...
- (void)setExtraInfo:(NSDictionary*)extraInfo ForItemWithKey:(NSString*)key {
  [_manifest setExtraInfo:extraInfo
           forItemWithKey:[NSDictionary sanitizeKey:key]];
}
...
/**
 * Check if the inputted manifest is valid, and not been tampered with based off
 * of our security delegate.
...
/**
 * Checks if the inputed data matches the signature thats stored in the the
 * manifest.
...
- (IonAsyncGenerationBlock)scannerBlockForKey:(NSString*)key
                           andGenerationBlock:
                               (IonAsyncGenerationBlock)genBlock {
  ...
  return ^(id data, IonResultBlock resultBlock) {
      ...
      // Validate that the data has not been manipulated by checking the
      // signature.
      if (![self dataObject:data matchesManifestItemWithKey:blockKey]) {
        resultBlock(
            // This should be an error, Past andrew was stupid...
            NULL);  // return null because the data appears to
                    // manipulated by an outside source.
...
- (void)setObject:(id)object
  ...
  // Do the action.
  [super setObject:objectData forKey:key withCompletion:completion];
}
...
- (void)removeObjectForKey:(NSString*)key
  ...
  [super removeObjectForKey:key withCompletion:completion];
}
...
- (void)removeAllObjects:(IonRawDataSourceCompletion)completion {
  ...
  [self deleteManifest:^(NSError* error) {
      ...
      [_manifest enumerateKeysUsingBlock:^(id key, BOOL* stop) {
          [self removeObjectForKey:[_manifest pathForItemWithKey:key]
                    withCompletion:NULL];
      }];
...
/**
 * This will report if we should cache the data generated with the specified
 * key.
 ...
 * @param {NSString*} the key to check.
 * @returns {BOOL} false if we should not cache it, true if we should or
 * Invalid.
 */
