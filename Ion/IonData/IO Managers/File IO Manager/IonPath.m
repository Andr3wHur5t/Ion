...
- (instancetype)init {
  self = [super init];
  if (self) _components = [NSArray array];
  return self;
}
 ...
 * @returns {instancetype} or NULL if invalid
 */
- (instancetype)initWithPath:(IonPath*)rootPath
          appendedByElements:(NSArray*)pathElements {
  if (!pathElements) return NULL;
  self = [self initFromPath:rootPath];
  if (self) [self appendHierarchy:pathElements];
  return self;
}
...
- (NSString*)itemName {
  ...
  if (![item isKindOfClass:[NSString class]]) return NULL;
...
- (IonPath*)pathAppendedByElements:(NSArray*)components {
  IonPath* newPath;
  if (!components || ![components isKindOfClass:[NSArray class]]) return NULL;
...
- (void)appendHierarchy:(NSArray*)additional {
  ...
  [self setComponentsArray:
            [IonPath normalizeComponents:
                         [self.components
                             arrayByAddingObjectsFromArray:additional]]];
}
...
- (void)setComponentsFromURL:(NSURL*)url {
  [self setComponentsArray:
            [IonPath normalizeComponents:[IonPath pathComponentsFromURL:url]]];
}
...
+ (NSArray*)pathComponentsFromURL:(NSURL*)url {
  ...
  intermediate =
      [url.path substringWithRange:NSMakeRange(1, url.path.length - 1)];
...
+ (NSArray*)normalizeComponents:(NSArray*)components {
  ...
  for (NSInteger i = [currentComponents count] - 1; i > 0; --i) {
    ...
    if (!component || ![component isKindOfClass:[NSString class]]) {
      ...
      if ([component isEqualToString:CurrentDirectoryKey] ||
      ...
      component = [normalizationExpresion
          stringByReplacingMatchesInString:component
                                   options:0
 ...
 */
+ (NSString*)stringFromComponents:(NSArray*)components
  ...
  for (NSString* component in normalizedComponents) {
    resultingPath = [resultingPath
        stringByAppendingFormat:@"%@%@",
                                isFirstItem ? @"" : DirectoryDelemiter,
                                component];
    isFirstItem = FALSE;
  }
...
+ (NSArray*)componentsFromURL:(NSURL*)url {
  if (!url || ![url isKindOfClass:[NSURL class]]) return NULL;
...
+ (NSArray*)componentsFromString:(NSString*)string {
  NSString* intermediate;
  NSArray* components;
  if (![string isKindOfClass:[NSString class]]) return NULL;
  ...
  if ([string characterAtIndex:0] == '/')
    intermediate =
        [intermediate stringByReplacingCharactersInRange:NSMakeRange(0, 1)
                                              withString:@""];

  intermediate =
      [intermediate substringWithRange:NSMakeRange(0, intermediate.length)];
...
/**
 * Gets a URL for the specified base directory, and creates it if it doesn't
 * already exists.
