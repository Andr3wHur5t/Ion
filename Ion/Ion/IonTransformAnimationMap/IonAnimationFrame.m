...
- (void) applyToView:(UIView *)view {
    ...
    if ( self.color )
        view.layer.backgroundColor = self.color.CGColor;
}

#pragma mark Composite Value Calculation
/**
 * Gets the resulting composite transformation for the frames state, and the inputted views size.
 ...
 */
+ (BOOL) automaticallyNotifiesObserversOfConfiguration { return FALSE; }
...
- (void) setConfiguration:(NSDictionary *)configuration {
    ...
    normalizedConfiguration =  [[[self class] defaultConfiguration] overriddenByDictionaryRecursively: configuration];
    
    // We are going to update our states with the inputted configuration inform for KVO compliance.
    [self willChangeValueForKey: @"configuration"];
    
    // Frame Metadata
    self.executionDelay = [[normalizedConfiguration numberForKey: sIonTransformAnimation_DelayKey] doubleValue];
    self.executionDuration = [[normalizedConfiguration numberForKey: sIonTransformAnimation_DurationKey] doubleValue];
    self.targetFrameName = [normalizedConfiguration stringForKey: sIonTransformAnimation_TargetTransformKey];
    
    // Frame Appearance Transformations
    self.color = [normalizedConfiguration colorFromKey: sIonTransformAnimation_ColorKey];
    ...
    // Debugging
    self.debugText = [normalizedConfiguration stringForKey: sIonTransformAnimation_LogKey];
    
    // We finished our configuration, we can reconstruct the configuration dictionary now.
...
- (NSDictionary *)configuration {
    return @{
             // Metadata
             sIonTransformAnimation_DelayKey: [NSNumber numberWithDouble: self.executionDelay],
             sIonTransformAnimation_DurationKey: [NSNumber numberWithDouble: self.executionDuration],
             sIonTransformAnimation_TargetTransformKey: self.targetFrameName ? self.targetFrameName : @"",
 ...
 */
+ (BOOL) automaticallyNotifiesObserversOfPosition { return FALSE; }
 ...
 * Switch KVO to manual mode.
 */
+ (BOOL) automaticallyNotifiesObserversOfRotation { return FALSE; }
 ...
 * Switch KVO to manual mode.
 */
+ (BOOL) automaticallyNotifiesObserversOfScale { return FALSE; }
