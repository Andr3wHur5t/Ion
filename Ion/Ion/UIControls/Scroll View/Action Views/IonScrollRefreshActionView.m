...
#pragma mark Guides
/**
 * Our content bottom guide; This is the calculated guide for the bottom size of the content view.
 */
@property (strong, nonatomic, readonly) IonGuideLine *contentBottomGuide;

/**
 * Our active height guide; This is just a guide set to the active height of the view.
 */
 ...
 * We simply add our self as a target action, and display a response to activation.
 */
...
- (instancetype) initWithContentView:(UIView *)contentView {
    return [self initWithContentView: contentView andActiveHeight: sIonRefreshActionViewDefault_ActiveHeight];
}
...
- (void) construct {
    ...
    self.canAutomaticallyDisplay = sIonRefreshActionViewDefault_CanAutomaticallyDisplay;
...
- (void) applyStyle:(IonStyle *)style {
    ...
    self.activeHeight = [[style.configuration numberForKey: sIonRefreshActionViewStyle_ActiveHeight
                                              defaultValue: @( sIonRefreshActionViewDefault_ActiveHeight)] floatValue];
    
    // Set activation distance or use default.
    self.activationDistance = [[style.configuration numberForKey: sIonRefreshActionViewStyle_ActivationDistance
                                                    defaultValue: @( sIonRefreshActionViewDefault_ActivationDistance)] floatValue];
}
...
+ (BOOL) automaticallyNotifiesObserversOfContentView { return FALSE; }
...
- (void) setContentView:(UIView *)contentView {
    ...
    if ( !_contentView || ![_contentView isKindOfClass: [UIView class]] )
        return;
    
    // Check if we can update the status of the gesture
    if ( [_contentView conformsToProtocol: @protocol(IonScrollRefreshActionStatusDelegate)] )
        self.scrollAction.statusDelegate = (UIView<IonScrollRefreshActionStatusDelegate> *)contentView;
...
- (IonGuideLine *)activeHeightGuide {
    if ( !_activeHeightGuide ) {
        _activeHeightGuide = [IonGuideLine guideWithTarget: self andKeyPath: @"activeHeight"];
...
- (void) show {
    ...
    if ( self.state != IonScrollRefreshActionViewState_Closed )
        return;
    
    // Animate if we have a scroll view.
    if ( ![self.superview isKindOfClass: [IonScrollView class]] )
        return;
    
    // Get the current insets
    _originalInsets = newInsets = self.scrollView.contentInset;
    
    // Modify
    newInsets.top += self.activeHeight + [self marginHeight].position;
...
- (void) hide {
    // Only animate if we are not already in open state.
    if ( self.state != IonScrollRefreshActionViewState_Open )
        return;
    ...
    // Set insets and update state
    self.scrollView.contentInset = _originalInsets;
    _state = IonScrollRefreshActionViewState_Closed;
}
...
- (void) refreshGestureDidOccur {
    if ( self.canAutomaticallyDisplay )
        [self showAnimated];
}

#pragma mark Active Height

- (void) setActivationDistance:(CGFloat)activationDistance {
    self.scrollAction.distance = activationDistance;
}

- (CGFloat) activationDistance {
    return self.scrollAction.distance;
}
